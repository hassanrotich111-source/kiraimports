...
const Exhibitions = () => {
  ...
  if (!servicesConfig.headline && servicesConfig.services.length === 0) return null;
  ...
  return (
    ...
    >
      ...
      <div ref={gridRef} className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-5 lg:gap-6">
        {servicesConfig.services.map((service: { id: number; title: string; image: string; date: string }) => (
          <div
            key={service.id}
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 w-full p-6 lg:p-8">
              <p className="museo-label text-white/50 mb-2 text-[10px]">{service.date}</p>
